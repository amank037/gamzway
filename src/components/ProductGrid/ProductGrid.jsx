import './ProductGrid.css'

function ProductGrid() {
  return (
    <div className='product-grid'>
      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="poker-card-container poker-card1">
              <img src="/ProductImages/poker/poker-card1.webp" alt="Poker Cards" className="stack-img" />
            </div>
            <div className="poker-card-container poker-card2">
              <img src="/ProductImages/poker/poker-card2.webp" alt="Poker Cards" className="stack-img" />
            </div>
            <div className="poker-card-container poker-card3">
              <img src="/ProductImages/poker/poker-card3.webp" alt="Poker Cards" className="stack-img" />
            </div>
            <div className="poker-card-container poker-card4">
              <img src="/ProductImages/poker/poker-card4.webp" alt="Poker Cards" className="stack-img" />
            </div>
            <div className="poker-chip-container poker-chip-left-top">
              <img src="/ProductImages/poker/poker-chip-left-top.webp" alt="Poker Chips Left" className="stack-img" />
            </div>
            <div className="poker-chip-container poker-chip-left-bottom">
              <img src="/ProductImages/poker/poker-chip-left-bottom.webp" alt="Poker Chips Left" className="stack-img" />
            </div>
            <div className="poker-chip-container poker-chip-right-top">
              <img src="/ProductImages/poker/poker-chip-right-top.webp" alt="Poker Chips Right" className="stack-img" />
            </div>
            <div className="poker-chip-container poker-chip-right-bottom">
              <img src="/ProductImages/poker/poker-chip-right-bottom.webp" alt="Poker Chips Right" className="stack-img" />
            </div>
            <h1 className='stack-img poker-stack-text product-title'>POKER</h1>
          </div>
        </a>
      </div>
      
      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="rummy-card-container1 rummy-card1">
              <img src="/ProductImages/Rummy/za-44.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container1 rummy-card2">
              <img src="/ProductImages/Rummy/za-45.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container1 rummy-card3">
              <img src="/ProductImages/Rummy/za-46.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container2 rummy-card4">
              <img src="/ProductImages/Rummy/za-48.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container2 rummy-card5">
              <img src="/ProductImages/Rummy/za-49.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container2 rummy-card6">
              <img src="/ProductImages/Rummy/za-50.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container2 rummy-card7">
              <img src="/ProductImages/Rummy/za-51.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container3 rummy-card8">
              <img src="/ProductImages/Rummy/za-52.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container3 rummy-card9">
              <img src="/ProductImages/Rummy/za-53.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container3 rummy-card10">
              <img src="/ProductImages/Rummy/za-54.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container4 rummy-card11">
              <img src="/ProductImages/Rummy/za-55.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container4 rummy-card12">
              <img src="/ProductImages/Rummy/za-56.webp" alt="" className="stack-img"/>
            </div>
            <div className="rummy-card-container4 rummy-card13">
              <img src="/ProductImages/Rummy/za-57.webp" alt="" className="stack-img"/>
            </div>
            <img src="/ProductImages/Rummy/za-43.webp" alt="" className="rummy-table stack-img"/>
            <h1 className='stack-img poker-stack-text product-title'>RUMMY</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="ludo-ghoti-container ludoghoti1">
              <img src="/ProductImages/ludo/ghoti blue.webp" alt="" className="stack-img"/>
            </div>
            <div className="ludo-ghoti-container ludoghoti2">
              <img src="/ProductImages/ludo/ghoti.webp" alt="" className="stack-img"/>
            </div>
            <div className="ludo-dice-container dice1">
              <img src="/ProductImages/ludo/za-59.webp" alt="" className="stack-img"/>
            </div>
            <div className="ludo-dice-container dice2">
              <img src="/ProductImages/ludo/za-58.webp" alt="" className="stack-img" />
            </div>
            <h1 className='stack-img poker-stack-text product-title'>LUDO</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="teen-card-container teen-card1">
              <img src="/ProductImages/Teen Patti/za-60.webp" alt="" className="stack-img"/>
            </div>
            <div className="teen-board-container teen-board">
              <img src="/ProductImages/Teen Patti/za-61.webp" alt="" className="stack-img"/>
            </div>
            <div className="teen-card-container teen-card2">
              <img src="/ProductImages/Teen Patti/za-62.webp" alt="" className="stack-img"/>
            </div>
            <div className="teen-card-container teen-card3">
              <img src="/ProductImages/Teen Patti/za-63.webp" alt="" className="stack-img"/>
            </div>
          </div>
          <h1 className='stack-img poker-stack-text product-title'>TEEN PATTI</h1>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="call-card-container call-card1">
              <img src="/ProductImages/Call Break/za-36.webp" alt="" className="stack-img"/>
            </div>
            <div className="call-card-container call-card2">
              <img src="/ProductImages/Call Break/za-37.webp" alt="" className="stack-img"/>
            </div>
            <div className="call-card-container call-card3">
              <img src="/ProductImages/Call Break/za-38.webp" alt="" className="stack-img" />
            </div>
            <div className="call-card-container call-card4">
              <img src="/ProductImages/Call Break/za-39.webp" alt="" className="stack-img" />
            </div>
            <h1 className='stack-img poker-stack-text product-title'>CALL BREAK</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="tambola-card-container tambola-card1">
              <img src="/ProductImages/Tambola/za-95.webp" alt="" className="stack-img"/>
            </div>
            <div className="tambola-card-container tambola-card2">
              <img src="/ProductImages/Tambola/za-96.webp" alt="" className="stack-img"/>
            </div>
            <div className="tambola-card-container tambola-card3">
              <img src="/ProductImages/Tambola/za-97.webp" alt="" className="stack-img" />
            </div>
            <h1 className='stack-img poker-stack-text product-title'>TAMBOLA</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="snake-card-container snake-card1">
              <img src="/ProductImages/Snake And ladder/za-40.webp" alt="" className="stack-img"/>
            </div>
            <div className="snake-card-container snake-card2">
              <img src="/ProductImages/Snake And ladder/za-41.webp" alt="" className="stack-img"/>
            </div>
            <div className="snake-board-container snake-board">
              <img src="/ProductImages/Snake And ladder/za-42.webp" alt="" className="stack-img" />
            </div>
            <h1 className='stack-img poker-stack-text product-title'>SNAKE LADDER</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="live-board-container">
              <img src="/ProductImages/Live Casino/Untitled-1.gif" alt="" className="stack-img"/>
            </div>
            <div className="live-chip-container live-chip1">
              <img src="/ProductImages/Live Casino/2-01.webp" alt="" className="stack-img"/>
            </div>
            <div className="live-chip-container live-chip2">
              <img src="/ProductImages/Live Casino/3-01.webp" alt="" className="stack-img"/>
            </div>
            <div className="live-chip-container live-chip3">
              <img src="/ProductImages/Live Casino/4-01.webp" alt="" className="stack-img"/>
            </div>
            <div className="live-chip-container live-chip4">
              <img src="/ProductImages/Live Casino/5-01.webp" alt="" className="stack-img"/>
            </div>
            <div className="live-card-container">
              <img src="/ProductImages/Live Casino/1-01.webp" alt="" className="stack-img"/>
            </div>
            <div className="woman-card">
              <img src="/ProductImages/Live Casino/2.webp" alt="" className="stack-img"/>
            </div>
            <h1 className='stack-img poker-stack-text product-title'>LIVE CASINO</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="aviator-card-container aviator-left-card1">
              <img src="/ProductImages/Avaitor/za-98.webp" alt="" className="stack-img"/>
            </div>
            <div className="aviator-card-container aviator-left-card2">
              <img src="/ProductImages/Avaitor/za-99.webp" alt="" className="stack-img"/>
            </div>
            <div className="aviator-card-container aviator-right-card1">
              <img src="/ProductImages/Avaitor/za-98.webp" alt="" className="stack-img"/>
            </div>
            <div className="aviator-card-container aviator-right-card2">
              <img src="/ProductImages/Avaitor/za-99.webp" alt="" className="stack-img"/>
            </div>

            <h1 className='stack-img poker-stack-text product-title'>AVIATOR</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="andar-card-container andar-card1">
              <img src="/ProductImages/Andar Bahar/za-64.webp" alt="" className="stack-img"/>
            </div>
            <div className="andar-card-container andar-card2">
              <img src="/ProductImages/Andar Bahar/za-65.webp" alt="" className="stack-img"/>
            </div>
          </div>
          <h1 className='stack-img poker-stack-text product-title'>ANDAR BAHAR</h1>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="blackjack-card-container blackjack-card1">
              <img src="/ProductImages/Black Jack/za-68.webp" alt="" className="stack-img"/>
            </div>
            <div className="blackjack-card-container blackjack-card2">
              <img src="/ProductImages/Black Jack/za-69.webp" alt="" className="stack-img"/>
            </div>
            <div className="blackjack-board-container blackjack-board">
              <img src="/ProductImages/Black Jack/za-67.webp" alt="" className="stack-img"/>
            </div>
          </div>
          <h1 className='stack-img poker-stack-text product-title'>BLACK JACK</h1>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="baccarat-card-container baccarat-card1">
              <img src="/ProductImages/Baccarat/za-71.webp" alt="" className="stack-img"/>
            </div>
            <div className="baccarat-card-container baccarat-card2">
              <img src="/ProductImages/Baccarat/za-70.webp" alt="" className="stack-img"/>
            </div>
          </div>
          <h1 className='stack-img poker-stack-text product-title'>BACCARAT</h1>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="dragon-img-container">
              <img src="/ProductImages/dragon tiger/2-01.webp" alt="" className="stack-img"/>
            </div>
            <div className="dragon-bg-container">
              <img src="/ProductImages/dragon tiger/1-01.webp" alt="" className="stack-img"/>
            </div>
          </div>
          <h1 className='stack-img poker-stack-text product-title'>DRAGON & TIGER</h1>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="roulette-chip-container roulette-chip1">
              <img src="/ProductImages/European Roulette/table.webp" alt="" className="stack-img"/>
            </div>
            <div className="roulette-board-container">
              <img src="/ProductImages/European Roulette/roulette.webp" alt="" className="stack-img  roulette-board"/>
              <div className="roulette-ball"></div>
            </div>
          </div>
          <h1 className='stack-img poker-stack-text product-title'>ROULETTE</h1>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="slots-card-container slots-card1">
              <img src="/ProductImages/Slots/coin 1.webp" alt="" className="stack-img"/>
            </div>
            <div className="slots-card-container slots-card2">
              <img src="/ProductImages/Slots/coin 2.webp" alt="" className="stack-img"/>
            </div>
            <div className="slots-card-container slots-card3">
              <img src="/ProductImages/Slots/coin 3.webp" alt="" className="stack-img"/>
            </div>
            {/* <div className="slots-card-container slots-card4">
              <img src="/ProductImages/Slots/coin 4.webp" alt="" className="stack-img"/>
            </div> */}
            <div className="slots-coin-container slots-coin1">
              <img src="/ProductImages/Slots/coin 1.webp" alt="" className="stack-img"/>
            </div>
            <div className="slots-coin-container slots-coin2">
              <img src="/ProductImages/Slots/coin 2.webp" alt="" className="stack-img"/>
            </div>
            <div className="slots-coin-container slots-coin3">
              <img src="/ProductImages/Slots/coin 3.webp" alt="" className="stack-img"/>
            </div>
            {/* <div className="slots-coin-container slots-coin4">
              <img src="/ProductImages/Slots/coin 4.webp" alt="" className="stack-img"/>
            </div> */}
            <div className="slots-coin-container slots-coin5">
              <img src="/ProductImages/Slots/coin 5.webp" alt="" className="stack-img"/>
            </div>
            <div className="slots-bg-container slots-bg">
              <img src="/ProductImages/Slots/bg.gif" alt="" className="stack-img"/>
            </div>

            <h1 className='stack-img poker-stack-text product-title'>SLOTS</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="sicbo-dice-container sicbo-dice1">
              <img src="/ProductImages/Sicbo/2-01.webp" alt="" className="stack-img sicbo-1"/>
            </div>
            <div className="sicbo-dice-container sicbo-dice2">
              <img src="/ProductImages/Sicbo/3-01.webp" alt="" className="stack-img sicbo-2"/>
            </div>
            <div className="sicbo-box-container sicbo-box">
              <img src="/ProductImages/Sicbo/1-01.webp" alt="" className="stack-img"/>
            </div>
            <h1 className='stack-img poker-stack-text product-title'>SICBO</h1>
          </div>
        </a>
      </div>
      
      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="keno-ball-container keno-ball1">
              <img src="/ProductImages/keno/1.webp" alt="" className="stack-img"/>
            </div>
            <div className="keno-ball-container keno-ball2">
              <img src="/ProductImages/keno/2.webp" alt="" className="stack-img"/>
            </div>
            <div className="keno-ball-container keno-ball3">
              <img src="/ProductImages/keno/3.webp" alt="" className="stack-img"/>
            </div>
            <h1 className='stack-img poker-stack-text product-title'>KENO</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="triple-ball-container triple-ball1">
              <img src="/ProductImages/Triple Chance/1.webp" alt="" className="stack-img"/>
            </div>
            <div className="triple-ball-container triple-ball2">
              <img src="/ProductImages/Triple Chance/2.webp" alt="" className="stack-img"/>
            </div>
            <div className="triple-ball-container triple-ball3">
              <img src="/ProductImages/Triple Chance/3.webp" alt="" className="stack-img"/>
            </div>
            <h1 className='stack-img poker-stack-text product-title'>TRIPLE CHANCE</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="pool-ball-container pool-ball1">
              <img src="/ProductImages/8 ball pool/1.webp" alt="" className="stack-img"/>
            </div>
            <div className="pool-ball-container pool-ball2">
              <img src="/ProductImages/8 ball pool/2.webp" alt="" className="stack-img"/>
            </div>
            <div className="pool-ball-container pool-ball3">
              <img src="/ProductImages/8 ball pool/3.webp" alt="" className="stack-img"/>
            </div>
            <div className="pool-ball-container pool-ball4">
              <img src="/ProductImages/8 ball pool/4.webp" alt="" className="stack-img"/>
              </div>
            <h1 className='stack-img poker-stack-text product-title'>8 BALL POOL</h1>
          </div>
        </a>
      </div>

      <div className='product-grid-item'>
        <a href="">
          <div className="image-stack">
            <img src="/ProductImages/poker/poker-bg.webp" alt="Poker bg" className="stack-img poker-stack-bg" />
            <img src="/ProductImages/poker/poker-glow.webp" alt="Poker Glow" className="stack-img poker-stack-glow" />
            <div className="horse-container">
              <img src="/ProductImages/Virtual Horse/1.gif" alt="" className="stack-img"/>
            </div>
            <h1 className='stack-img poker-stack-text product-title'>VIRTUAL HORSE</h1>
          </div>
        </a>
      </div>
      
    </div>
  )
}

export default ProductGrid